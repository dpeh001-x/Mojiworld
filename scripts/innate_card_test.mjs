// The U panel's bottom row: the Innate Growth card and the talent cards, as pop + cute stickers.
//
// Per user: "Innate growth can be much better, cool pop and cute mix", then "job talent should also be
// redesigned to match". Both cards were rebuilt, so this pins that every number survived and that
// the talent card's behaviour did too:
...
//   5. the card wears the candy rim (a border-box gradient) and the titles are Fredoka
...
const read = (st) => page.evaluate(async (st) => {
  ...
  return {
    ...
    rim: (cs.backgroundImage.match(/gradient/g) || []).length >= 3, titleFont: title ? getComputedStyle(title).fontFamily : '',
...
if (A) {
  ...
  ok('5. the candy rim (a border-box gradient) and a Fredoka title', A.rim && /Fredoka/.test(A.titleFont), JSON.stringify([A.rim, A.titleFont]));
