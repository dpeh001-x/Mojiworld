...
const R = await page.evaluate(() => {
  ...
  // -- 1b. FULL freeze: nothing at all reaches the paused player -----------
  // v0.29.672 gated damage only; measured leaks were hitStun 140, knockback
  // vx 1.2, poison 8000 ms and 1000 ms of i-frames burned. A scorpion is the
  // sharpest probe: contact + a poison-on-hit trait.
  {
    game.monsters.length = 0; game.hazards.length = 0; game.projectiles.length = 0;
    player.level = 40; player.maxHp = 5000; player.hp = 5000;
    player.invulnerable = 0; player.hitStun = 0; player.vx = 0; player.vy = 0;
    player._poisonTimer = 0; player._slowTimer = 0; player.burnTimer = 0;
    player.x = 900; player.y = gp.y - player.h;
    const sc = spawnMonster(900, gp.y - 60, 'scorpion', false, false);
    const x0 = player.x;
    game.paused = true;
    for (let i = 0; i < 300; i++) {
      game.time++;
      if (sc) { sc.x = player.x; sc.y = player.y; sc.facing = 1; sc.vx = 0; sc.vy = 0; }
      if (typeof _lxCoopWorldStep === 'function') _lxCoopWorldStep(16.667);
    }
    game.paused = false;
    ok('a paused player is not stunned', (player.hitStun | 0) === 0, `hitStun ${player.hitStun | 0}`);
    ok('a paused player is not knocked back', Math.abs(player.vx) < 0.01 && Math.abs(player.x - x0) < 0.5,
       `vx ${player.vx.toFixed(2)}, moved ${(player.x - x0).toFixed(1)} px`);
    ok('a paused player is not poisoned', (player._poisonTimer | 0) === 0, `poison ${player._poisonTimer | 0} ms`);
    ok('a paused player burns no i-frames', (player.invulnerable | 0) === 0, `invulnerable ${player.invulnerable | 0}`);
    game.monsters.length = 0;
  }
  // the host's direct boss strike is also a no-op while paused
  {
    player.hp = 5000; player.hitStun = 0; player.invulnerable = 0; player.vx = 0;
    const savedHost = net.isHost, savedHostId = net.hostId, savedFollow = (typeof _coopFollowingHost === 'function') ? _coopFollowingHost : null;
    try {
      net.isHost = false; net.hostId = 2;
      // eslint-disable-next-line no-global-assign
      _coopFollowingHost = () => true;
      game.paused = true;
      _coopApplyBossHit({ id: 2, map: game.currentMap, d: 2000, x: player.x + player.w / 2, y: player.y, r: 500 });
      game.paused = false;
      ok('the host\'s boss strike is a no-op on a paused guest',
         player.hp === 5000 && (player.hitStun | 0) === 0 && Math.abs(player.vx) < 0.01,
         `hp ${player.hp}, hitStun ${player.hitStun | 0}, vx ${player.vx.toFixed(2)}`);
      _coopApplyBossHit({ id: 2, map: game.currentMap, d: 2000, x: player.x + player.w / 2, y: player.y, r: 500 });
      ok('the same strike still lands when NOT paused', player.hp < 5000, `hp -> ${player.hp}`);
    } finally {
      net.isHost = savedHost; net.hostId = savedHostId;
      if (savedFollow) _coopFollowingHost = savedFollow;
      game.paused = false;
      // the unpaused strike drives hp deep negative — revive fully so no
      // later assertion inherits a dead/dying player (the flake class that
      // bit this suite twice already)
      game.dying = false; player.hp = 5000; player.hitStun = 0; player.invulnerable = 0; player.vx = 0;
    }
  }
