// CHANGELOG.html entry for v0.30.284. Newest on top, under </header>.
...
if (s.includes('>v0.30.284 <')) { console.log('already applied'); process.exit(0); }
...
const ENTRY = J(
  '</header>',
  '',
  '<h2>v0.30.284 <span class="tag"><span class="pill balance">balance</span> Necromancer &amp; Hexmaster tamed: G, B and the summons</span></h2>',
  '<p><b>Version note:</b> this change first pushed as v0.30.284&rsquo;s twin numbered v0.30.283 and, in doing so, briefly clobbered a parallel session&rsquo;s v0.30.283 (&ldquo;six per-frame wastes deleted from the update loop&rdquo;) &mdash; the same whole-file race the sync rules exist for. Their diff re-applied cleanly alongside this nerf; both suites (their 8-check loop smoke test, this entry&rsquo;s 5-check nerf suite) pass on the merged file, and this entry takes the next number.</p>',
...
renameSync(F + '.tmp', F);
console.log(`applied: CHANGELOG v0.30.284 (+${grew} chars)`);
