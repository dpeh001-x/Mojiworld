#!/usr/bin/env node
// Gravitos's RIFT RING: the translucent energy ring that frames his teleport warning, and its nine-frame loop (ludo.ai).
// =============================================================================
// Per user, on the v0.30.858 warning (a screenshot of the plain violet oval around the destination rift): "The violet
// ring is too simple, generate something more aesthetic, make it transluscent".
//
// A circular halo of swirling violet-magenta cosmic energy - wispy nebula strands, a thin bright inner edge, small
// stars caught in it - with an EMPTY centre, so the game can lay it around the rift (stretched into the rift's upright
// oval) and fold copies of it inward around Gravitos. No dark cel outline (a glow band with an ink rim reads as a hoop,
// not light); the game draws it additively at partial alpha, so the scene shows through it. Same seat / feather / gates
// as gen_gravitos_voidrift.mjs; every loop step must move.
//
//   node scripts/gen_gravitos_riftring.mjs              # dry run: prints the briefs
//   node scripts/gen_gravitos_riftring.mjs --generate   # needs LUDO_API_KEY; writes the base + 9 frames
...
const PROMPT =
  'game vfx sprite, a MAGIC ENERGY RING seen straight on: a thin circular halo of swirling violet and magenta cosmic '
  + 'energy, made of wispy flowing nebula strands and fine luminous streaks that spiral around the circle, a thin bright '
  + 'lavender-white glowing inner edge, tiny white stars and sparkles caught in the swirl, soft glow fading outward. The '
  + 'ring fills most of the picture and is perfectly round, and its CENTRE IS COMPLETELY EMPTY and transparent - nothing '
  + 'inside the ring at all. Ethereal, delicate and translucent, like light and mist rather than a solid hoop. Colours '
  + 'ONLY: violet, purple, magenta, orchid pink, lavender, white highlights. No green, no yellow, no orange, no gold, no '
  + 'cyan, no blue. Pure transparent background, alpha only: no ground, no floor, no scene, no box, no frame, no square, '
  + 'no border, no solid outline, no character, no text, no letters, no watermark.';
const MOTION =
  'the energy strands flow slowly around the ring in one direction, the nebula wisps drift and shimmer, the bright inner '
  + 'edge pulses gently brighter and dimmer, sparkles twinkle and new ones appear; the ring stays exactly in place, keeps '
  + 'its round shape and size, and the centre stays empty; seamless loop, nothing leaves the frame';
...
export const OUTLINE = { px: 4, r: 0x1c, g: 0x08, b: 0x2a, inset: 0.93 };   // a deep violet-black contour
...
async function seat(raw) {
  ...
  const f = await feather(canvas);   // no ink outline: a glow band with a dark rim reads as a hoop
