...
//   node scripts/gen_lava.mjs --generate      # generate both
//   flags: --force --only lava_drop
...
const PREFIX = 'Chibi anime game VFX sprite for a 2D platformer in the Mojiworld aesthetic. ' +
  ...
  'Soft painterly cel-shaded anime style, bold clean edges, vibrant saturated colors, strong additive glow. ' +
...
const VFX = {
  lava_drop: 'a falling blob of MOLTEN LAVA dropping straight DOWN: a rounded glowing teardrop of bright yellow-white molten core fading to orange then deep red at the edges, with a couple of thin dripping tendrils and a few hot sparks/embers trailing upward, intense incandescent heat glow, vertical falling shape.',
  lava_pool: 'a bubbling MOLTEN LAVA puddle seen TOP-DOWN: a glowing pool of liquid rock, bright yellow-orange cracks of molten heat across a darker red-black crust, rising round bubbles and faint heat shimmer, a circular flat disc shape.',
};
...
if (!has('--generate')) {
  console.log(`# ${keys.length} lava VFX -> Sprites/vfx/<file>.webp:\n`);
...
console.log(`Generating ${keys.length} lava VFX (force:${force})...`);
