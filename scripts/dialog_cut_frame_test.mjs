...
try {
  ...
  // v0.30.979 - the name is set in Fredoka now, Cinzel the fallback (the family string above is cut at 20 chars)
  check(/rgb\((12, 11, 16|23, 16, 42)\)/.test(r.name.fill) && /linear-gradient/.test(r.name.bg) && /Fredoka|Cinzel/.test(r.name.family) && (r.portrait.figure ? r.name.clip === 'none' : (r.name.transform !== 'none' && /polygon/.test(r.name.clip))), 'live: the name is ink lettering on a paper plate - a flat tag beside a figure, a tilted banner beside the medallion', J(r.name));
