...
//   node scripts/generate_boss_walk_anim.mjs --only kingKrook --generate
...
const WALK_OVERRIDES = {
  // v0.29.138 — the generic walk collapsed to a standing pose (feet planted in
  // every frame). Force an exaggerated visible march.
  towerArbiter: 'the gold-armoured arbiter knight MARCHES in place (treadmill) with an EXAGGERATED, clearly visible cartoon march: in every mid-stride frame ONE KNEE IS LIFTED HIGH with the foot fully OFF THE GROUND while the other leg plants, then they alternate; the torso bobs up-down with each step, the purple plume and cape swing with the rhythm, and his greatsword stays held low in one hand, swaying slightly. The legs MUST visibly move — absolutely NOT a standing pose, NOT a subtle shuffle, NOT feet planted. He does NOT travel or slide across the frame — he stays centered. CRITICAL: the BODY stays the EXACT same size and scale in EVERY frame — no zoom, no rescale. ONE single connected body; do NOT duplicate or detach any limbs or the sword.' + FACING,
  // v0.29.136 — new art: quadruped brick-built stegosaurus (green bricks, red back plates).
  legosaurus: 'the brick-built toy stegosaurus LUMBERS in place (treadmill) — a heavy four-legged walk cycle: the thick brick legs take clear alternating stomps (diagonal pairs like a real quadruped), the chunky body rocks with each step, the red back-plates and tail sway with the rhythm. It stays a FOUR-LEGGED quadruped: do NOT stand it up on two legs, do NOT add arms. It does NOT travel or slide across the frame — it stays centered. CRITICAL: the BODY stays the EXACT same size and scale in EVERY frame — no zoom, no rescale; the brick construction stays rigid and intact. ONE single connected body; do NOT duplicate or detach any limbs.' + FACING,
  // v0.29.136 — KrookKing (red lizard king, replaces the old turtle art): a fat
  // croc who knuckle-walks — heavy lumbering gait, not an upright stride.
  kingKrook: 'the chubby red crocodile-lizard king LUMBERS in place (treadmill) — a heavy, weighty four-limbed walk cycle: his thick clawed legs take big alternating stomps while his knuckled forearms swing and plant in opposition, the big cream belly swaying with each heavy step, his purple fur-trimmed royal cape swishing and his thick tail swaying behind; the gold crown stays firmly on his head. The step motion must be CLEAR and heavy — not a tiny sway. It does NOT travel or slide across the frame — it stays centered. CRITICAL: the BODY stays the EXACT same size and scale in EVERY frame — no zoom, no rescale. ONE single connected body; do NOT duplicate or detach any limbs.' + FACING,
  // v0.29.x — Mooma (new art): stubby nub-feet, so a waddle, not a stride.
  mooma: 'the big cute mushroom WADDLES in place (treadmill) — it rocks gently side to side on its little stubby nub-feet taking small alternating steps, the chubby cream body bobbing and squishing with each step and the large pink cap bouncing softly with the rhythm, its tiny stubby arms swinging a little. It does NOT travel or slide across the frame — it stays centered. CRITICAL: the BODY and CAP stay the EXACT same size and scale in EVERY frame — no zoom, no rescale. ONE single connected body; do NOT duplicate or detach any limbs.' + FACING,
