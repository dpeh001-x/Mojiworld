...
// background can still be better, the pink should be more sparse, look at AAA pop art".
//
...
//
//   plate    ink black, halftone ramps into the top-left and bottom-right corners, the boon card's keyline
//   shard    one asymmetric paper-white burst behind him - two long spikes on the diagonal, the rest
//            short - inked, shaded with a Ben-Day ramp toward the lower right, printed off an acid-yellow
//            copy, with a thin paper outline echo of itself behind (the layered plates of a P5 menu)
//   accents  four white motion slashes along the diagonal, a yellow star; the only pink is one spike's misprint sliver
//            and one small star
...
const INK = '#0b0a0e', PAPER = '#f4f1ea', PK = '#ff2d95', AY = '#f3f542', KEY = '#3a3842';
...
function plateSvg() {
  return `<rect width="${S}" height="${S}" fill="${INK}"/>${ramp(14, -0.707, -0.707, 120, 200, 5.8, PAPER, 0.2)}${ramp(14, 0.707, 0.707, 170, 170, 5.8, PAPER, 0.2)}
...
function shardSvg() { const P = shardPts(), [a, r] = SPIKES[1], i = 1;
  ...
  return `<defs><clipPath id="sc"><polygon points="${poly(P)}"/></clipPath></defs>
    ...
    <polygon points="${poly(P)}" transform="translate(${CX} ${CY}) scale(1.16) translate(${-CX} ${-CY})" fill="none" stroke="${PAPER}" stroke-width="3" stroke-linejoin="round" opacity="0.9"/>
    <polygon points="${poly(P)}" transform="translate(13 11)" fill="${AY}" stroke="${INK}" stroke-width="6" stroke-linejoin="round"/>
    <polygon points="${poly(P)}" fill="${PAPER}" stroke="${INK}" stroke-width="7" stroke-linejoin="round"/>
    <g clip-path="url(#sc)">${ramp(9, 0.707, 0.707, 10, 230, 4.4, INK, 0.85)}</g>`; }
...
function accentSvg() { const slash = (x, y, len, w) => { const ux = 0.643, uy = -0.766, nx = -uy, ny = ux;   // along the long spikes, tapering
    ...
    return `<polygon points="${poly([[x - ux * len / 2, y - uy * len / 2], [ex + nx * w / 2, ey + ny * w / 2], [ex - nx * w / 2, ey - ny * w / 2]])}" fill="${PAPER}"/>`; };
