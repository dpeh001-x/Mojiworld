// Build Sprites/bosses/walk/gravitos_0..8.webp from a 3x3 magenta-keyed sheet,
// normalized to the IDLE set's calibration (content height + baseline + center).
import sharp from 'sharp';
sharp.cache(false);
const SHEET = process.argv[2];
const OUTDIR = 'Sprites/bosses/walk';
...
for (let i = 0; i < 9; i++) {
  ...
  await sharp({ create: { width: CANVAS_W, height: CANVAS_H, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } })
    ...
    .webp({ quality: 92 }).toFile(`${OUTDIR}/gravitos_${i}.webp`);
  console.log('frame', i, 'content', box.w + 'x' + box.h, '-> scale', scale.toFixed(3), 'placed at', left + ',' + top);
}
console.log('DONE — 9 walk frames written, calibrated to idle ref');
