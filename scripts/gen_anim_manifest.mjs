  ...
  try {
    ...
    for (let y = 0; y < H; y++) {
      ...
      for (let x = 0; x < W; x++) {
        ...
        if (a > 64) { any = true; if (a > 235) { solid = true; break; } }
      }
...
for (const [t, ent] of Object.entries(manifest)) {
  ...
  // v0.29.185 — CROSS-STATE validation: every state must share the base canvas
  // (walk sets used to ship on Ludo's padded 990 canvas → per-state sizeFactor
  // + anchor breakage). Fix with scripts/rebase_anim_canvas.mjs.
  for (const st of Object.keys(ent.states)) {
    const s = ent.states[st];
    if (s.w !== b.w || s.h !== b.h) {
      _mismatches++;
      console.warn(`!! BOSS STATE CANVAS MISMATCH: ${t} ${st} ${s.w}x${s.h} vs base ${b.w}x${b.h}` +
        ` — run: node scripts/rebase_anim_canvas.mjs --apply --only ${t}`);
    }
  }
}
// v0.29.185 — CHARACTER-DRIFT validation for rebased bosses: after
// rebase_anim_canvas.mjs, every frame's content height should sit within ~2%
// of the boss's idle median (per-frame character lock baked into the pixels).
// Only enforced for bosses previously rebased — others rely on the runtime
// content-norm by design.
for (const t of ['legosaurus', 'mooma', 'kingKrook', 'towerArbiter']) {
  const ent = manifest[t];
  if (!ent || !ent.states.idle || !ent.states.idle.cb) continue;
  const med = (a) => { const s = [...a].sort((x, y) => x - y); return s[s.length >> 1]; };
  const ref = med(ent.states.idle.cb.filter(Boolean).map(cb => (cb[1] - cb[0] + 1) / ent.states.idle.h));
  for (const st of Object.keys(ent.states)) {
    const s = ent.states[st];
    if (!s.cb) continue;
    s.cb.forEach((cb, i) => {
      if (!cb) return;
      const dev = Math.abs((cb[1] - cb[0] + 1) / s.h - ref) / ref * 100;
      if (dev > 2) { _mismatches++; console.warn(`!! CHARACTER DRIFT: ${t} ${st}[${i}] content-h ${dev.toFixed(1)}% off idle median — re-run rebase_anim_canvas.mjs.`); }
    });
  }
}
if (_mismatches) console.warn(`!! ${_mismatches} boss canvas/drift issue(s) — fix the asset(s), do not rely on the runtime guard.`);
else console.log('canvas validation: all boss statics + states + rebased character heights are consistent.');
