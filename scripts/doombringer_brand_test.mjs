...
// connect varies with machine load (observed 2-4 under parallel test runs).
// This check exists to prove the WIRING - a real cast brands and stokes at all
// - not to count landed strikes; the deterministic tag arithmetic above
// already pins the per-hit numbers.
check(r.integStacks >= 1 && r.integHeat >= 2, 'a real Blade of Calamity cast brands the target and stokes heat', { stacks: r.integStacks, heat: r.integHeat });
