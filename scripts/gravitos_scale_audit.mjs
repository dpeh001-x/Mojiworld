...
for (const key of KEYS) {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    // Deviation in EITHER direction. Tracking only the max ratio reported a
    // perfect "1.00x" for a set containing frames drawn 20% too SMALL — the
    // gate caught them via the per-frame flags, but the headline number said
    // the set was flawless, which is the kind of reassuring summary that hides
    // the defect it is meant to surface.
    const dev = Math.max(rL, rT, 1 / rL, 1 / rT);
