...
add('L. Loops & exploits', 'L4', 'The prologue ends early only on a real stall', S.fixed, 'v0.30.863 (per owner): the one 120 s wall-clock cap became a stall watchdog. It force-finishes only after 45 s with no scene change, clip frame or fight-clock tick while the game is visible; a stanza card waiting for the player never counts (up to 10 min a card); a forced finish closes every clip cover through its own Skip button and the chained scenes no longer open (prologue_cap_test).');
...
add('L. Loops & exploits', 'L6', 'Repeatable-reward loops are closed', S.fixed, 'v0.30.861 / v0.30.862 (per owner): (1) the Cursed Chest is seeded per (map, day) and opens once per map per day; (2) Nightmare and zodiac echoes get the refight gear terms - budget 1, 40% of the chance, one live roll per 10 play-minutes per boss; (3) a day that goes backwards pays no login bonus, challenge, parcel, streak milestone or Boss Rush reward; (4) a summon\'s kill pays and counts nothing after 2 minutes without player input; (5) a co-op guest pays a host kill only for a monster it mirrored, capped by its own monster table (loops_fix_test, coop_kill_trust_test). Also v0.30.864: quick-sell confirms in the game\'s own dialog, the last player-facing browser confirm.');
