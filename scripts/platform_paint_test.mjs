...
try {
  ...
  const r = await page.evaluate((THEMES) => {
    ...
    const stats = (cv, w, h, ground) => {
      ...
      return { size: W + 'x' + Hc, wantH: P + Math.max(Math.ceil(h), G.depth) + 3, capLum: capN ? capL / capN : 0, bodyLum: bodyN ? bodyL / bodyN : 0, opaque: inside ? opaque / inside : 0, decor, under, solid, solid1, air, keyLum: kmin, edgeLum };
    };
