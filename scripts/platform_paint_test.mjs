...
// map; terrain v3 (after MapleStory and the AAA side-scrollers) gives floating platforms mass
// below their one-way collision box, an overhanging lip, material, light and a biome underside.
// This bakes platforms and grounds through the real _cutePlatformSprite and reads the pixels back:
// the bake is the size the art needs, the surface is exactly at the collision line, the cap is
// lighter than the face, the box is opaque, the keyline is dark, islands hang below and grounds do
// not, decor stands on the surface, variants differ, bakes are cached, and the colour is the map's.
...
try {
  ...
  const r = await page.evaluate((THEMES) => {
    ...
    const stats = (cv, w, h, ground) => {
      ...
      // the surface: at five columns the collision line is solid and 3px above it is (mostly) air
      let solid = 0, air = 0;
      for (const f of [0.25, 0.4, 0.5, 0.6, 0.75]) { const x = Math.round(2 + w * f); if (d[at(x, P + 1) + 3] > 200) solid++; if (d[at(x, P - 3) + 3] < 40) air++; }
      // the keyline down the face's left edge, below the cap's hanging edge
      const ky = P + Math.round(G.cap + 4); let kmin = 255; for (let x = 1; x <= 8; x++) { const i = at(x, Math.min(Hc - 1, ky)); if (d[i + 3] > 150) kmin = Math.min(kmin, lum(d, i)); }
      return { size: W + 'x' + Hc, wantH: P + Math.max(Math.ceil(h), G.depth) + 3, capLum: capN ? capL / capN : 0, bodyLum: bodyN ? bodyL / bodyN : 0, opaque: inside ? opaque / inside : 0, decor, under, solid, air, keyLum: kmin };
    };
    ...
    const va = _cutePlatformSprite(200, 12, tint, false, 'grass', 0), vb = _cutePlatformSprite(200, 12, tint, false, 'grass', 1);
    ...
    const capRGB = (cv) => { const d = cv.getContext('2d').getImageData(10, P + 3, cv.width - 20, 2).data; let r = 0, g = 0, b = 0, n = 0; for (let i = 0; i < d.length; i += 4) if (d[i + 3] > 200) { r += d[i]; g += d[i + 1]; b += d[i + 2]; n++; } return n ? [r / n | 0, g / n | 0, b / n | 0] : null; };
  ...
  for (const th of THEMES) {
    ...
    ok(`${th}: both bakes exist, as deep as the art (platform ${t.plat && t.plat.size}, ground ${t.ground && t.ground.size})`, t.plat && t.ground && t.plat.size === '124x' + t.plat.wantH && t.ground.size === '404x' + t.ground.wantH, { want: [t.plat && t.plat.wantH, t.ground && t.ground.wantH] });
    ok(`${th}: the surface is exactly at the collision line (platform and ground)`, t.plat.solid === 5 && t.plat.air >= 3 && t.ground.solid === 5 && t.ground.air >= 3, { plat: [t.plat.solid, t.plat.air], ground: [t.ground.solid, t.ground.air] });
