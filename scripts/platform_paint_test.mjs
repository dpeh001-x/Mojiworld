...
// v0.30.636 - bakes are made at device resolution (cv._lxLW/_lxLH carry the logical box); every
...
try {
  ...
  const r = await page.evaluate((THEMES) => {
    ...
    // a device-resolution bake, read back at its logical box (a pre-v0.30.636 bake is returned as it is)
    const logical = (cv) => {
