...
export function parseReport(text, source) {
  ...
  for (const raw of text.split('\n')) {
    ...
    // "[skill sound]" items come from the Skill Sound Tuner (tools/skill_sfx_tester.html).
    const item = line.match(/^\s*\d+\.\s+(.+?)\s+\[(hit sound|death sound|NPC voice|fallback sound|skill sound)\]\s*$/);
