...
Additional authored `bg_v3_*` files are registered in `mojiworld_game.html` for candy, bubblegum, celestial, underwater, zodiac, boss, and carriage maps.
...
Find the map definition in `mojiworld_game.html` and add or change its `bg` field:
...
- Classic visuals: `mojiworld_game.html`
- Cinematic visuals: `mojiworld_game.html?art=cinematic`
- Force classic: `mojiworld_game.html?art=classic`
