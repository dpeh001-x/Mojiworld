...
`mojiworld_game.html`, warn if any changed hunk falls outside the
...
# Inclusive line ranges in mojiworld_game.html that are mobile-only.
...
def main():
    ...
    if not fp.endswith("mojiworld_game.html"):
        sys.exit(0)
    ...
    try:
        diff = subprocess.check_output(
            ["git", "diff", "--unified=0", "mojiworld_game.html"],
