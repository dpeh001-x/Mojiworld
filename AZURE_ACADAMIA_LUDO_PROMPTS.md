...
4. If a sprite reads too tall/short next to the others, add an entry to `NPC_SPRITE_HEIGHTS` in `mojiworld_game.html` (default render height is 78 px; Sage Mira uses 115 px). Hera in particular may want a taller height (e.g. 100 px) to match her archmage stature.
