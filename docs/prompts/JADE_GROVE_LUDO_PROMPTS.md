...
| NPC | File path |
| --- | --- |
| Lady Hong | `Sprites/npc/lady_hongyu.webp` |
...
4. If a sprite reads too tall/short next to the others, add an entry to `NPC_SPRITE_HEIGHTS` in `mojiworld_game.html` (default render height is 78 px; Sage Mira uses 115 px). Lady Hong may want a taller height to match her descended-deity stature.
