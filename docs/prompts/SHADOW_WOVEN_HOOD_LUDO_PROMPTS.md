# Shadow-Woven Hood — Ludo.ai sprite prompts (v0.25.411)

Three NPCs live in the new map (right of Sweet Candy Canyon, World Map ▶ Shadow-Woven Hood). The game already references the sprite filenames in `NPC_SPRITE_FILES` — drop the generated webps into `Sprites/npc/` and they auto-load on next page reload, replacing the procedural fallback.
...
- **Style anchor**: match the existing detailed-illustrative pack — Fashionista / Sage Mira / Brok have the right look. Paste the master style line into every prompt.
...
4. If a sprite reads too tall/short next to the others, add an entry to `NPC_SPRITE_HEIGHTS` in `mojiworld_game.html` (default render height is 78 px; Sage Mira uses 115 px because she's a sky-elder).
