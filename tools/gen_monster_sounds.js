...
const HIT_OVERRIDE = {
  // v0.29.x — per user: "rotter hurt and death noise sounds wrong, its like a
  // animal squeak". Classification again, exactly like future_lyra below.
  // familyFor('zombie') tests /skel|undead|crypt|sepulchre|ribcage/ for the
  // undead branch — `zombie` matches NONE of those words, so the Rotter fell
  // through to `beast` and was generated from "a quick gruff ANIMAL grunt" /
  // "a feral ANIMAL yelp ... as a BEAST is felled". A squeak is what that asks
  // for. Overridden here rather than by adding an `undead` family, because the
  // runtime's family fallback only resolves names that have mob_<family>_*.mp3
  // on disk (see _MONSTER_SFX_FILES) — a new family name would resolve to
  // silence for anything without a custom clip.
  // Shipped audio is generated by scripts/gen_rotter_sfx.mjs, which also
  // enforces the < 1 s bar and a measured brightness ceiling; these strings are
  // kept in step with it so a whole-roster regen reproduces the same character.
  zombie: 'a rotting corpse taking a hit — a dull wet thud into waterlogged dead flesh with a low '
    + 'phlegmy grunt forced out of a ruined throat, thick and gurgling. '
    + 'NOT an animal, NOT a squeak, NOT a yelp, nothing high-pitched',
...
const DIE_OVERRIDE = {
  // v0.29.x — the Rotter, see the matching HIT_OVERRIDE above for the root
  // cause. The wet vocabulary is deliberately absent: "gurgling / fluid /
  // squelching splat" describe broadband spray, and every take built from
  // those words measured BRIGHTER than the squeak it replaced (3216 Hz, then
  // 2689 Hz, against 2604 Hz incumbent). Described as a chest-cavity groan
  // plus a body-fall — both low-frequency events — it landed at 186 Hz.
  zombie: 'a corpse collapsing — a deep hollow dying groan from the chest, muffled as if forced '
    + 'through rotten lungs, pitch sagging downward and running out of air, ending in the dull '
    + 'soft thud of a heavy body dropping onto earth. Muffled, bassy, dark. '
    + 'NO splash, NO gurgle, NO hiss, nothing bright or trebly. '
    + 'NOT an animal, NOT a yelp or whimper',
